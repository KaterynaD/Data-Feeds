drop view if exists fsbi_dw_spinn.VPROPERTY_ALLCOV;		
create view fsbi_dw_spinn.VPROPERTY_ALLCOV as 		
SELECT  modeldata_id		
       , systemidstart		
       , systemidend		
       ...
       , case when pe.renewaltermcd='1 Year' then 1 else 2 end * CovA_wp*(DateDiff(d, startdate, enddate)/365.25) CovA_ep		
       , case when pe.renewaltermcd='1 Year' then 1 else 2 end * CovB_wp*(DateDiff(d, startdate, enddate)/365.25) CovB_ep		
       , case when pe.renewaltermcd='1 Year' then 1 else 2 end * CovC_wp*(DateDiff(d, startdate, enddate)/365.25) CovC_ep		
       , case when pe.renewaltermcd='1 Year' then 1 else 2 end * CovD_wp*(DateDiff(d, startdate, enddate)/365.25) CovD_ep		
       , case when pe.renewaltermcd='1 Year' then 1 else 2 end * CovE_wp*(DateDiff(d, startdate, enddate)/365.25) CovE_ep		
       , case when pe.renewaltermcd='1 Year' then 1 else 2 end * CovF_wp*(DateDiff(d, startdate, enddate)/365.25) CovF_ep		
/*------------------------------------------POLICY DATA---------------------------------*/	   	
...
grant select,references on fsbi_dw_spinn.VPROPERTY_ALLCOV to pespagnet,ksun;		
